...
public class Scheduler implements SchedulerClient {
    ...
    protected boolean startTask(Task task) {
        if (task != null && task instanceof OnStartup) {
            return startOnStartup((OnStartup) task);
        }
        return false;
    }

    protected boolean startOnStartup(OnStartup os) {
        if (os == null) {
            return false;
        }
        try {
    ...
    protected void executeOnStartup() {
        List<OnStartup> r = scheduleRepository.getOnStartups(onStartup);
        if (r == null) {
            return;
        }
        r.forEach(os -> {
